...
object StaxXmlParserUtils {
  ...
  def gatherRootAttributes(parser: XMLEventReader): Array[Attribute] = {
    ...
    rootEvent.asStartElement.getAttributes.asInstanceOf[java.util.Iterator[javax.xml.stream.events.Attribute]].asScala.toArray
  }
    ...
    do {
      parser.nextEvent match {
        case e: StartElement =>
          ...
          e.getAttributes.asInstanceOf[java.util.Iterator[javax.xml.stream.events.Attribute]].asScala.foreach { att =>
            xmlString
              .append(' ')
              .append(att.getName)
              .append("=\"")
