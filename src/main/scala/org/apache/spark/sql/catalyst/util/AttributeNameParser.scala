...
trait AttributeNameParser {
  ...
  /** Used to split attribute name by dot with backticks rule. Backticks must appear in pairs, and the quoted string
    * must be a complete name part, which means `ab..c`e.f is not allowed. We can use backtick only inside quoted name
    * parts.
    */
