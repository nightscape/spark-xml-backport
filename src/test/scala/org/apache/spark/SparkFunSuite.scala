...
/** Base abstract class for all unit tests in Spark for handling common functionality.
  *
  * Thread audit happens normally here automatically when a new test suite created. The only prerequisite for that is
  * that the test class must extend [[SparkFunSuite]].
  *
  * It is possible to override the default thread audit behavior by setting enableAutoThreadAudit to false and manually
  * calling the audit methods, if desired. For example:
  *
...
// scalastyle:on
  ...
  override protected def test(testName: String, testTags: Tag*)(testBody: => Any)(implicit pos: Position): Unit = {
  ...
  /** Note: this method doesn't support `BeforeAndAfter`. You must use `BeforeAndAfterEach` to set up and tear down
    * resources.
    */
  ...
  /** Log the suite name and the test name before and after each test.
    *
    * Subclasses should never override this method. If they wish to run custom code before and after each test, they
    * should mix in the {{org.scalatest.BeforeAndAfter}} trait instead.
    */
  ...
  /** Creates a temporary directory containing a secret file, which is then passed to `f` and will be deleted after `f`
    * returns.
    */
  ...
  /** Adds a log appender and optionally sets a log level to the root logger or the logger with the specified name, then
    * executes the specified function, and in the end removes the log appender and restores the log level if necessary.
    */
  ...
  /** Sets all configurations specified in `pairs` in SparkEnv SparkConf, calls `f`, and then restores all
    * configurations.
    */
  ...
  /** Checks an exception with an error class against expected results.
    ...
    * @param errorClass
    *   The expected error class identifying the error
    * @param sqlState
  ...
  protected def checkError(exception: SparkThrowable, errorClass: String, sqlState: String): Unit =
    checkError(exception, errorClass, Some(sqlState))
  ...
  protected def checkError(
    ...
    checkError(exception, errorClass, Some(sqlState), false, Array(context))

  protected def checkError(exception: SparkThrowable, errorClass: String, context: ExpectedContext): Unit =
    checkError(exception, errorClass, None, false, Array(context))
  ...
  protected def checkError(
    ...
    checkError(exception, errorClass, sqlState, false, Array(context))
  ...
  protected def checkErrorMatchPVals(
    ...
    checkError(exception, errorClass, None, matchPVals = true, parameters = parameters)
  ...
  protected def checkErrorMatchPVals(
    ...
    checkError(exception, errorClass, sqlState, matchPVals = true, Array(context))
  ...
  protected def checkErrorTableNotFound(
    ...
    checkError(exception = exception, errorClass = "TABLE_OR_VIEW_NOT_FOUND", queryContext = Array(queryContext))

  protected def checkErrorTableNotFound(exception: SparkThrowable, tableName: String): Unit =
    checkError(exception = exception, errorClass = "TABLE_OR_VIEW_NOT_FOUND")

  protected def checkErrorTableAlreadyExists(exception: SparkThrowable, tableName: String): Unit =
    checkError(exception = exception, errorClass = "TABLE_OR_VIEW_ALREADY_EXISTS")
  ...
  object ExpectedContext {
    ...
    def apply(fragment: String, callSitePattern: String): ExpectedContext = {
      new ExpectedContext("", "", -1, -1, fragment, callSitePattern)
    }
  }
      ...
      if (copyEvent.getLevel.isMoreSpecificThan(_threshold)) {
        _loggingEvents.synchronized {
          if (_loggingEvents.size >= maxEvents) {
            ...
            throw new IllegalStateException(s"Number of events reached the limit of $maxEvents$loggingInfo")
          }
