...
trait CodegenInterpretedPlanTest extends PlanTest {
  ...
  override protected def test(testName: String, testTags: Tag*)(
    ...
    super.test(testName + " (interpreted path)", testTags: _*)(
      withSQLConf(SQLConf.CODEGEN_FACTORY_MODE.key -> interpretedMode) { testFun }
    )(pos)
  }

  protected def testFallback(testName: String, testTags: Tag*)(testFun: => Any)(implicit pos: source.Position): Unit = {
...
trait PlanTestBase extends PredicateHelper with SQLHelper with SQLConfHelper { self: Suite =>
  ...
  /** Normalizes plans:
    *   - Filter the filter conditions that appear in a plan. For instance, ((expr 1 && expr 2) && expr 3), (expr 1 &&
    *     expr 2 && expr 3), (expr 3 && (expr 1 && expr 2) etc., will all now be equivalent.
    ...
    plan transform {
      ...
      case Project(projectList, child) =>
        val projList = projectList
          .map { e =>
            e.transformUp { case g: GetViewColumnByNameAndOrdinal =>
              g.copy(viewDDL = None)
            }
          }
  ...
  protected def comparePlans(plan1: LogicalPlan, plan2: LogicalPlan, checkAnalysis: Boolean = true): Unit = {
    if (checkAnalysis) {
    ...
    if (normalized1 != normalized2) {
      fail(s"""
          ...
          rewriteNameFromAttrNullability(normalized2).treeString
        ).mkString("\n")}
         """.stripMargin)
    }
  }
