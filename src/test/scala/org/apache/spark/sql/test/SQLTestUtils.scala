...
/** Helper trait that should be extended by all SQL test suites within the Spark code base.
  *
  * This allows subclasses to plugin a custom `SparkSession`. It comes with test data prepared in advance as well as all
  * implicit conversions used extensively by dataframes. To use implicit methods, import `testImplicits._` instead of
  * through the `SparkSession`.
  *
  * Subclasses should *not* create `SparkSession`s in the test suite constructor, which is prone to leaving multiple
  * overlapping [[org.apache.spark.SparkContext]]s in the same JVM.
  */
private[sql] trait SQLTestUtils extends SparkFunSuite with SQLTestUtilsBase with PlanTest {
  ...
  /** Materialize the test data immediately after the `SQLContext` is set up. This is necessary if the data is accessed
    * by name but not through direct reference.
    */
  protected def setupTestData(): Unit = {
    loadTestDataBeforeTests = true
  }

  /** Disable stdout and stderr when running the test. To not output the logs to the console, ConsoleAppender's `follow`
    * should be set to `true` so that it will honor reassignments of System.out or System.err. Otherwise,
    * ConsoleAppender will still output to the console even if we change System.out and System.err.
    */
  ...
  /** Run a test on a separate `UninterruptibleThread`.
    */
  protected def testWithUninterruptibleThread(name: String, quietly: Boolean = false)(body: => Unit): Unit = {
  ...
  /** Copy file in jar's resource to a temp file, then pass it to `f`. This function is used to make `f` can use the
    * path of temp file(e.g. file:/), instead of path of jar's resource which starts with 'jar:file:/'
    */
  ...
  /** Creates the specified number of temporary directories, which is then passed to `f` and will be deleted after `f`
    * returns.
    */
...
/** Helper trait that can be extended by all external SQL test suites.
  *
  * This allows subclasses to plugin a custom `SQLContext`. To use implicit methods, import `testImplicits._` instead of
  * through the `SQLContext`.
  *
  * Subclasses should *not* create `SQLContext`s in the test suite constructor, which is prone to leaving multiple
  * overlapping [[org.apache.spark.SparkContext]]s in the same JVM.
  */
private[sql] trait SQLTestUtilsBase extends Eventually with BeforeAndAfterAll with SQLTestData with PlanTestBase {
  self: Suite =>
  ...
  /** A helper object for importing SQL implicits.
    *
    * Note that the alternative of importing `spark.implicits._` is not possible here. This is because we create the
    * `SparkSession` immediately before the first test is run, but the implicits import is needed in the constructor.
    */
  ...
  protected def uncacheTable(tableName: String): Unit = {
    ...
    spark.sharedState.cacheManager.uncacheQuery(
      spark,
      spark.table(tableName).queryExecution.logical,
      cascade = cascade,
      blocking = true
    )
  }

  /** Creates a temporary database and switches current database to it before executing `f`. This database is dropped
    * after `f` returns.
    *
  ...
  /** Enables Locale `language` before executing `f`, then switches back to the default locale of JVM after `f` returns.
    */
  ...
  /** Activates database `db` before executing `f`, then switches back to `default` database after `f` returns.
    */
  ...
  /** Turn a logical plan into a `DataFrame`. This should be removed once we have an easier way to construct `DataFrame`
    * directly out of local data without relying on implicits.
    */
  protected implicit def logicalPlanToSparkQuery(plan: LogicalPlan): DataFrame = {
    Dataset.ofRows(spark, plan)
  }

  /** This method is used to make the given path qualified, when a path does not contain a scheme, this path will not be
    * changed after the default FileSystem is changed.
    */
...
private[sql] object SQLTestUtils {
  ...
  def compareAnswers(sparkAnswer: Seq[Row], expectedAnswer: Seq[Row], sort: Boolean): Option[String] = {
