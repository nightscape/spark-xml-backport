...
abstract class QueryTest extends PlanTest {
  ...
  /** Runs the plan and makes sure the answer matches the expected result.
    ...
    *   the [[DataFrame]] to be executed
    * @param expectedAnswer
    *   the expected result in a [[Array]] of [[Row]]s.
    */
  protected def checkAnswer(df: => DataFrame, expectedAnswer: Array[Row]): Unit = {
    checkAnswer(df, expectedAnswer.toSeq)
  }
  ...
  protected def checkAggregatesWithTol(dataFrame: DataFrame, expectedAnswer: Row, absTol: Double): Unit = {
  ...
  /** Asserts that a given [[Dataset]] will be executed using the cache with the given name and storage level.
    */
  ...
  protected def getNextLineCallSitePattern(lines: Int = 1): String = {
    val cs = Thread.currentThread().getStackTrace()(2)
    Pattern.quote(s"${cs.getClassName}.${cs.getMethodName}(${cs.getFileName}:${cs.getLineNumber + lines})")
  }
}
...
object QueryTest extends Assertions {
  ...
  /** Runs the plan and makes sure the answer matches the expected result. If there was exception during the execution
    * or the contents of the DataFrame does not match the expected result, an error message will be returned. Otherwise,
  ...
  private def genError(expectedAnswer: Seq[Row], sparkAnswer: Seq[Row], isSorted: Boolean = false): String = {
  ...
  def sameRows(expectedAnswer: Seq[Row], sparkAnswer: Seq[Row], isSorted: Boolean = false): Option[String] = {
